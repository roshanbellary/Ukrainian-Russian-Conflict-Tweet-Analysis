...
graph2 <- ggplot(data=twt_cnt, aes(y=country,x=nplot))+geom_bar(stat="identity",fill="#222222")+scale_x_continuous(n.breaks = 15,expand = c(0, 0))+scale_y_discrete(limits=twt_cnt$country)+labs(y="Country", x="Tweet Count",title="Country Tweet Count")+theme(plot.title = element_text(hjust = 0.5,face="bold"),plot.background = element_rect(fill = "#E5E5E6"),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
graph2
```